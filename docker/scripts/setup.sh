...
if ! wp core is-installed --network && [ $WORDPRESS_MULTISITE = 1 ]; then
  ...
  # Ensure correct permissions for files / directories created during install.
  if [ -z "$SKIP_CHOWN" ]; then
    chown -Rf nginx.nginx /var/www/html
  fi
fi

# Check for single site installation
